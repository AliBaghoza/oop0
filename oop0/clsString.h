...
public:
    ...
    //28 Invert All String Letters Case
    ...
    static vector<string> SplitPunctuationDelim(string S1)
    {
        ...
        // use find() function to get the position of the delimiters   
        while ((pos = searchPunctuations(S1)) !=-1)
        {
            sWord = S1.substr(0, pos); // store the word    
            if (sWord != "")
            {
    ...
    static short searchPunctuations(string S1)
    {
        string S2 = "";
        for (short i = 0; i < S1.length(); i++)
        {
